...
def download(url, params, filename):
  ...
  try:
    ...
    if json_response["Status"]["Code"] != 0:
      return { "ok": False, "code": "request_invalid", "message": json_response["Status"]["Content"] }
  except:
    pass
...
def parse_raw_data(filename):
  ...
  df["month"] = df.month.apply(lambda month: datetime.datetime.strptime(month, "%B").date().month)
...
def main():
  ...
  most_recent_entry = response.json()
  curr_month = datetime.date(most_recent_entry["year"], most_recent_entry["month"], 1)
  ...
  download_response = download(GENESIS_API_URL, params=genesis_query_params, filename=filename)  
